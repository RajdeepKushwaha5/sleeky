...
export function SiteFooter() {
  ...
  const copyPhone = async () => {
    await navigator.clipboard.writeText("+91 7073526113");
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="flex flex-col gap-4 rounded-2xl border border-border bg-card/80 p-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={copyEmail}
              className="group relative flex h-10 w-10 items-center justify-center rounded-full border border-border bg-card/60 transition-all hover:bg-foreground/10"
              title={emailTooltip}
            >
              <Mail className="h-4 w-4 text-foreground/60 group-hover:text-foreground" />
              <span className="pointer-events-none absolute -top-10 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-lg bg-foreground px-3 py-1.5 text-xs text-background opacity-0 transition-opacity group-hover:opacity-100">
                {emailTooltip}
              </span>
            </button>
            ...
            <a
              ...
              title="LinkedIn"
            >
              <svg className="h-4 w-4 text-foreground/60" fill="currentColor" viewBox="0 0 24 24">
